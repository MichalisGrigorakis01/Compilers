...
Symbol* newtemp(void) {
    ...
    symInsert(table, sym->name, "TEMP VAR", "", yylineno, scope, 0);
...
expr* emit_iftableitem(expr* e) {
    if(e->type != tableitem_e){
        //printf("[DEBUG] emit_iftableitem: SKIPPED — not a tableitem (type = %d)\n", e->type);
        return e;
    }else{
...
expr* emit_or(expr* e1, expr* e2, int* really_useful_index_ptr) {
    ...
    if (e1->type == boolexpr_e && e2->type == boolexpr_e) {
        ...
        patchlist(e1->falselist, *really_useful_index_ptr);
