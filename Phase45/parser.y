%{
	...
	char function_name_stack[10][200];
	int function_name_top = -1;
	...
	expr* get_expression(char* token_id, int CONCERN){
		symTable* entry = NULL;
	...
	int handle_token(int type /*0 funx, 1 var, 2 arg, 3 library funx*/){
		if(type==0){ /*in case of function*/
			...
			symTable* inserted = symInsert(table, prev_id, "LIBRARY FUNCTION", arglist, 0, scope, 1);
			
			// Set syminfo right here
			symTable* entry = symSearch(inserted, prev_id, "LIBRARY FUNCTION", scope);
			if (entry && !entry->syminfo) {
				Symbol* sym = malloc(sizeof(Symbol));
				sym->type = libraryfunc_s;
				sym->name = strdup(prev_id);
			...
			namedLibfuncs[totalNamedLibfuncs++] = strdup(prev_id);
			return 0;
		}
...
assignexpr:	lvalue ASSIGN {
								assign_lhs_expr = $1;
								if (!assign_lhs_expr || !assign_lhs_expr->sym || !assign_lhs_expr->sym->name) {
									fprintf(stderr, "[ASSIGNEXPR] ERROR: Cannot assign to invalid lvalue at line %d\n", yylineno);
									exit(1);
								}
								assign_id_index +=1;
								...
								if(strcmp(prev_id,assign_id[assign_id_index])!=0){ //checking if prev_id has changed, this means that the other end of the assignment has a variable (else its a constant)
										int result = declaration_check(1,0,1); //checking if the variable exists
										int result2 = declaration_check(0,0,1); //checking if the function exists
								...
								$$ = temp;
								assign_lhs_expr = NULL;
		}
		;

primary:	lvalue 				{
									//$$ = get_expression(prev_id,0);
									...
									symTable* entry = NULL;
									if(!$$){
											handle_token(1); //if the variable is not found in the symtable, the assignment wants it to be inserted anyway,
											// but there is no way of knowing if its a variable or function, so i suppose as a variable.
											if(scope ==0 )
													entry = symSearch(table, prev_id,"GLOBAL VARIABLE", scope);
											else
													entry = symSearch(table, prev_id,"LOCAL VARIABLE", scope);
											if(entry){
															Symbol* sym = extractSymbol(entry);
															$$= lvalue_expr(sym);
													}
													...
													$$  = NULL;
													printf("[primary]: ERRROR: Couldnt insert undefined variable (%s) to symtable\n", prev_id);
											}
									}
...
lvalue:	ID			
			{	
				...
				if (!entry) entry = symSearch(table, yytext, "LIBRARY FUNCTION", scope);

				if (entry) {
					Symbol* sym = extractSymbol(entry);
					$$ = lvalue_expr(sym);
				} else{					// Not found — we must insert it (probably a variable)
					handle_token(1);  // insert as variable

					if (scope == 0)
						entry = symSearch(table, yytext, "GLOBAL VARIABLE", 0);
					else
						entry = symSearch(table, yytext, "LOCAL VARIABLE", scope);

					if (entry) {
						Symbol* sym = extractSymbol(entry);
						$$ = lvalue_expr(sym);
					} else {
						//fprintf(stderr, "[LVALUE] ERROR: Could not create symbol for '%s'\n", yytext);
						$$ = NULL;
					}
				}
			}
...
call:	call L_BRACKET elist R_BRACKET	{
	...
	| lvalue 
		{
			if(!is_member)	
				strcpy(function_name_stack[++function_name_top], prev_id); /*the prev_id will have stored the latest id, this could be argument, so store the functions name at once to use it later at insertion*/
		} 
			callsuffix
			{
				...
				is_function_result =1;
				if(!is_member){
					strcpy(prev_id, function_name_stack[function_name_top--]);
					...
					//expr* func = $1;
    				//expr* args = $3;
					expr* func_e = get_expression(prev_id,1);
					//expr* func_e = $1;
					if (!func_e) {
...
elist:	 expr {declaration_check(3,0,1); $$ = emit_iftableitem($1); }
...
funcdef:    FUNCTION ID {
							...
							if(dont_do_it==0){
								...
								//expr* func_e = get_expression($2,1);
								expr* func_e = newexpr(programfunc_e);
								symTable* func_entry = symSearch(table, $2, "USER FUNCTION", scope);
								Symbol* sym = extractSymbol(func_entry);
								sym->type = programfunc_s;
								sym->taddress = currInstr;
								func_entry->syminfo = sym;
...
anon_funcdef:	 FUNCTION L_BRACKET {scope+=1;} idlist {
									...
									//expr* func_e = get_expression(prev_id,1);
									expr* func_e = newexpr(programfunc_e);
									func_e->sym = extractSymbol(symSearch(table, prev_id, "USER FUNCTION", scope));
									emit(funcstart, NULL, NULL, func_e, 0, yylineno);
									...
									//phase3
									expr* func_e = get_expression(funcdef_names[inside_function_def],1);
									int funcend_quad_index = nextquad();
									emit(funcend, NULL, NULL, func_e, 0, yylineno);
...
int main(int argc,char** argv){
	...
	namedLibfuncs = malloc(sizeof(char*) * 256);  // Allocate space for 256 libfunc names
	if (!namedLibfuncs) {
		fprintf(stderr, "[FATAL] Failed to allocate memory for namedLibfuncs\n");
		exit(1);
	}
	...
	for(int i=0; i<12; i++){
		strcpy(prev_id,  funx_names[i]);//strcpy(prev_id, (char*)(funx_names+i));
		handle_token(3);
	}
	...
	printf("\n");
	init_stack(&s);
	extract_global_variables(&s, table);
	print_stack(&s);
	printf("\n");
	...
	print_instructions();
	generate_binary("output.abc");


	//Phase 5
	
    load_binary("output.abc");
	initialize_avm();
	pc = 0;
